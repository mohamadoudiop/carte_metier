<?php
    ...
    $sql2 = "SELECT cm.idCarteMareyeur, cm.NumeroCarte, cm.LibelleCategorie,
                    r.NumQuitance, r.DateQuitance, MontantRenouv, DateDebutRenouv, DateFinRenouv, RenewalYear
...
<head>
    ...
    <title>Liste des renouvellements - Gestion des cartes de mareyeur</title>
...
<div class="main-content container-fluid">
    <div class="page-title">
        <div class="row">
            <div class="col-12 col-md-6 order-md-1 order-last">
                <h3>Liste des renouvellements</h3>
            </div>
            <div class="col-12 col-md-6 order-md-2 order-first">
                <nav aria-label="breadcrumb" class='breadcrumb-header'>
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="tableau_de_bord.php">Tableau de bord</a></li>
                        <li class="breadcrumb-item active" aria-current="page">Liste des renouvellements</li>
                    </ol>
                </nav>
            </div>
        </div>
    ...
    <section class="section">
        <div class="card">
            <form action="export_renouv.php" method="post">
                <div class="card-body">
                    <button type="submit" name="export_renouv" class="btn waves-effect waves-light btn-primary">Exporter</button>
                    <table class='table table-striped' id="table1">
                        <thead>
                            <tr>
                                ...
                                <th>Date quitance</th>
                                <th>Montant Renouvellement</th>
                                <th>Date début Renouvellement</th>
                                <th>Date Fin Renouvellement</th>
                                <th>Année renouvelée</th>
                                <th>Modification</th>
                            </tr>
                        </thead>
                        <tbody>
                            ...
                            <?php
                                if ($result2 && $result2->num_rows > 0) {
                                    ...
                                    while ($row = $result2->fetch_assoc()) {
                                        ...
                                        echo "<td>" . $row['DateQuitance'] . "</td>";
                                        echo "<td>" . $row['MontantRenouv'] . "</td>";
                                        echo "<td>" . $row['DateDebutRenouv'] . "</td>";
                                        echo "<td>" . $row['DateFinRenouv'] . "</td>";
                                        echo "<td>" . $row['RenewalYear'] . "</td>";
                                        // Colonne "Modifier" avec lien vers la page de modification (modifier.php)
                                        echo "<td><a class='badge-link' href='modifierRenouvellement.php?idCarteMareyeur=" . $row['idCarteMareyeur'] . "'><span class='btn btn-warning'>Modifier</span></a></td>";
                                        echo "</tr>";
                                    }
                                } else {
                                    echo "<tr><td colspan='12'>Renouvellement enregistré.</td></tr>";
                                }
                            ?>
                        </tbody>
                    </table>
